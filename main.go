...
func main() {
	...
	commandName := flag.String("c", "", "Command: avi2mp4, 2k, 1080, addlogo, to4kfolder, to2kfolder, to1080folder, iwara")

	flag.Var(&inputFiles, "i", "Input file path (can be repeated)")
	flag.Parse()

	if len(inputFiles) == 0 || *commandName == "" {
		fmt.Println("Usage: app -i <file1> -i <file2> -c <avi2mp4|2k|1080|addlogo|to4kfolder|to2kfolder|to1080folder|iwara>")
		os.Exit(1)
	}
